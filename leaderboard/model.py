...
def player_rating_updater(repository):
    ...
    def build_player_updater(ranking):
        return player_rating_updater(repository.find_player_by_pseudo(ranking.pseudo), ranking.rank)

    def retrieve_players(rankings):
        return [build_player_updater(ranking) for ranking in rankings]
